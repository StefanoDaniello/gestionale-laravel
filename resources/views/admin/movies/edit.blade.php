...
<div class="container mt-5">
    ...
    <form action="{{route('admin.movies.update', $movie->slug)}}"  class="row g-3 mt-3" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="col-6">
            ...
            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title"
             name="title" value="{{ old('title' , $movie->title ) }}" min="3" max="255" >
        ...
        <div class="col-6">
            ...
            <input type="text" class="form-control @error('producer') is-invalid @enderror" id="producer" name="producer"
             name="producer" value="{{ old('producer', $movie->producer) }}" min="3" max="255" >
            <div id="nameHelp" class="form-text">Inserire minimo 3 caratteri e massimo 255</div>
            @error('producer')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="col-6">
            ...
            <input type="date" class="form-control @error('release_date') is-invalid @enderror" id="release_date" name="release_date"
             name="release_date" value="{{ old('release_date', $movie->release_date) }}" min="3" max="255" >
            @error('release_date')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-6">
            ...
            <input type="number" class="form-control @error('rt_score') is-invalid @enderror" id="rt_score" name="rt_score"
             name="rt_score" value="{{ old('rt_score', $movie->rt_score) }}" min="1" max="5" >
            @error('rt_score')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="col-6">
            ...
            <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image"
             name="image" value="{{ old('image', $movie->image) }}" min="3" max="255">
            @error('image')
            <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
